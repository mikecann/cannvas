...
- Build the mirror with `pnpm build:mirror` (kiosk build into `dist/`, needs `VITE_CANNVAS_DEVICE_TOKEN` in `.env.local` matching `CANNVAS_DEVICE_TOKEN` on the production Convex deployment). Never publish `dist/` anywhere public: Cloudflare serves `dist-public/` from `pnpm build:public`.
